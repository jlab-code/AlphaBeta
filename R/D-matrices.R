...
#' @export
#'
dMatrices<-function(directory, cytosines, posteriorMaxFilter ){
  # checking errors
  ...
  posteriorMaxFilter <- as.numeric(posteriorMaxFilter)
  if (posteriorMaxFilter < 0 | posteriorMaxFilter > 1 ){
    stop("posteriorMax value must be < 1 and > 0. ")
  }
  data.names<-list.files(paste0(dirname(directory),"/",basename(directory)),pattern = "*.txt", full.names = TRUE)

  ds_pairs<-combn(data.names, 2)
  print(ds_pairs)
  final_ds <- runMatrix((length(ds_pairs)/2), cytosines, pMFilter)
...
# getting file names
#-------------------------------------------------------------
getNames <- function(nameA ){
  tmp_name_A<-gsub(pattern = "\\.*[^G^0-9.L-]", "", basename(nameA))
  A=gsub(pattern = "\\-.*", "",tmp_name_A)
  AA<- gsub(pattern = "\\.*[^G0-9]", "-L", basename(A))
  return(AA)
}
runMatrix <- function(pairs, cytosines, pMFilter){
  flag=TRUE
  for (i in 1:pairs){ #(length(ds_pairs)/2)
      in_file <-ds_pairs[,i]
      name_ds<- getNames(in_file[1])
      name_ds[2]<- getNames(in_file[2])
      cat(paste0("Running: ",name_ds[[1]], " and ",name_ds[[2]], " ( ", i , " out of ",length(ds_pairs)/2," )"),"\n")

      cat("Constructing data-frame ...\n")
      file_A <- fread(in_file[1], skip = 0, sep = '\t',select=c("seqnames","start","strand","context","posteriorMax","status"))
      file_B <- fread(in_file[2], skip = 0, sep = '\t',select=c("seqnames","start","strand","context","posteriorMax","status"))
