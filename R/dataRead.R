...
DM.dataRead <- function(dFile, cytosine, posteriorMaxFilter){
  ...
  command <- sprintf(
    paste0("grep --text -w ",cytosine , " %s" ),
    paste(dFile, collapse = "")
  )

  if(.Platform$OS.type == "unix") {

    tmp <- fread(cmd = command, select = c("V1","V2","V3","V4","V7","V8"))
    cnames<-c("seqnames","start","strand","context","posteriorMax","status")
    colnames(tmp) <- cnames

  }else{

    tmp <- fread(dFile, skip = 0, sep = '\t',
  ...
  tmp <- tmp %>% filter(tmp$posteriorMax >= posteriorMaxFilter & tmp$seqnames !="M" &  tmp$seqnames!="C")
...
RC.dataRead <- function(dFile, cytosine, posteriorMaxFilter){
  ...
  if(.Platform$OS.type == "unix") {
    ...
    cnames<-c("seqnames","context","posteriorMax","rc.meth.lvl")
    colnames(tmp) <- cnames
